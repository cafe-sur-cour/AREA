...
/**
 * @swagger
 * /api/microsoft/unsubscribe:
 *   post:
 *     summary: Unsubscribe from Microsoft events
 *     tags:
 *       - Microsoft Service
 *     description: |
 *       Unsubscribe user from Microsoft events. OAuth connection remains intact.
 *       NOTE: Microsoft webhook implementation is not yet complete.
 *     security:
 *       - bearerAuth: []
 *     responses:
...
router.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return res.status(200).json({
        message: 'Successfully unsubscribed from Microsoft events',
        subscription: {
          subscribed: subscription.subscribed,
          unsubscribed_at: subscription.unsubscribed_at,
          service: subscription.service,
        },
        note: 'Microsoft webhook cleanup will be implemented when webhooks are ready',
      });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ error: 'Internal Server Error' });
