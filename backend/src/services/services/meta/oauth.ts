...
export class FacebookOAuth {
	...
	constructor() {
		...
		this.facebookApiBaseUrl = process.env.SERVICE_FACEBOOK_API_BASE_URL || 'https://graph.facebook.com';
		this.facebookAuthBaseUrl = process.env.SERVICE_FACEBOOK_AUTH_BASE_URL || 'https://www.facebook.com';
	...
	async exchangeCodeForToken(code: string): Promise<FacebookTokenResponse> {
		...
		if (!response.ok) {
			throw new Error(`Facebook OAuth token exchange failed: ${response.statusText}`);
		}

		const data = (await response.json()) as FacebookTokenResponse | FacebookErrorResponse;
		if ('error' in data) {
			throw new Error(`Facebook OAuth error: ${(data as FacebookErrorResponse).error.message}`);
		}
	...
	async getUserInfo(accessToken: string): Promise<FacebookUser> {
		...
		const response = await fetch(`${this.facebookApiBaseUrl}/me?${params.toString()}`, {
			method: 'GET',
			headers: {
				Accept: 'application/json',
			},
		...
		if (!response.ok) {
			throw new Error(`Failed to get Facebook user info: ${response.statusText}`);
		}
		return (await response.json()) as FacebookUser;
	}

	async storeUserToken(userId: number, tokenData: FacebookTokenResponse): Promise<void> {
