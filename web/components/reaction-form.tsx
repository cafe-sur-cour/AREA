...
interface ReactionFormProps {
  ...
  reactionsConfig: Record<string, any>[];
...
}: ReactionFormProps)
{
    ...
    const [reactionInstances, setReactionInstances] = useState<ReactionInstance[]>([
        {
    ...
    useEffect(() => {
        const fetchAction = async () => {
            try {
                setIsLoading(true);
                const res = (await api.get<{ services: ServiceReaction[] }>({ endpoint: "/services/reactions" })).data;
    ...
    const removeReaction = (id: string) => {
        const updatedInstances = reactionInstances.filter(instance => instance.id !== id);
    ...
    const updateReactionInstance = (id: string, updates: Partial<ReactionInstance>) => {
    ...
    const getAvailableReactions = (serviceId: string): Reaction[] => {
        return ServiceReactions.find(service => service.id === serviceId)?.reactions || [];
    };
                        ...
                        {instance.selectedService && (
                            <div>
                                ...
                                <Select 
                                    ...
                                    onValueChange={(value) => {
                                        const selectedReaction = getAvailableReactions(instance.selectedService!)
                                            .find(reaction => reaction.id === value) || null;
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <SelectGroup>
                                            ...
                                            {getAvailableReactions(instance.selectedService).map((reaction) => (
