...
function TestComponent() {
  const { user, isAuthenticated, isLoading, logout, refreshUserInfo } =
    useAuth();
  ...
  return (
    <div>
      ...
      <button data-testid='refresh' onClick={refreshUserInfo}>
        Refresh
      </button>
    </div>
  );
...
describe('AuthProvider', () => {
  ...
  test('✅ utilise le cache local valide', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('user')).toHaveTextContent('CachedUser')
    );
  });
  ...
  test('✅ cache expiré -> recharge depuis API', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('user')).toHaveTextContent('FreshUser')
    );
  });
  ...
  test('✅ cache corrompu -> supprimé', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('user')).toHaveTextContent('FixedUser')
    );
  ...
  test('✅ logout remet user à null', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('user')).toHaveTextContent('UserToLogout')
    );
